...
predation <- function(pred, inds, xcol = 2, ycol = 3, rcol = 4, dcol = 5){
  ...
  for (p in 1:predators){
    ...
    yloc <- pred[p, ycol]
    N.prey <- sum(inds[,xcol] == xcol & inds[, ycol] == yloc)  #sums all the prey on p's location
    if(N.prey > 0){
      pred[p, dcol] <- 0   #The predator does not die
    }
    if(N.prey > 1){
      pred[p, rcol] <- 1   #Predator reproduces
    }
    # Unfortunately something also happens to the prey: -------------------
    if(N.prey > 0) {
      prey <- which(inds[, xcol] == xloc & inds[, ycol] == yloc)   #marks all places were predator p and prey coexist
      if(N.prey > 2){
  ...
  new.pred[, 1] <- rnorm(n = nrow(pred), mean = 20, sd = 2.5)  #This approximates a lynx ish
  new.pred[, 2] <- sample(x = 1:8, size = nrow(pred), replace = T)
  new.pred[, 3] <- sample(x = 1:8, size = nrow(pred), replace = T)
